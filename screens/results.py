...
class ResultsScreen:
    def __init__(self, game, region, hits, total):
        ...
        self.font_boss = pygame.font.SysFont("Arial", 20, bold=True)
        self.font_boss_sm = pygame.font.SysFont("Arial", 15)
        ...
        import random
        self.confetti = [
            (random.randint(0, WIDTH), random.uniform(-HEIGHT, 0),
    ...
    def _repeat_phase(self):
        self.game.boss_just_unlocked = False
        self.game.go_to_challenge(self.region)
    ...
    def handle_events(self):
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if self.btn_map.collidepoint(event.pos):
                    self._leave_to_map()
                if self.btn_rep.collidepoint(event.pos):
                    self._repeat_phase()
    ...
    def update(self):
        ...
        if pct >= 0.7:
            ...
            self.confetti = [
                (x, y if y < HEIGHT else -20, vx, vy, c)
                for x, y, vx, vy, c in self.confetti
            ]
    ...
    def draw(self):
        ...
        if self.game.boss_just_unlocked:
            ...
            pygame.draw.rect(s, GREEN, box, 2, border_radius=14)

            t1 = self.font_boss.render("CHEFÃO LIBERADO!", True, YELLOW)
            t2 = self.font_boss_sm.render(
                "Você concluiu todas as fases e atingiu a pontuação mínima.",
                True, WHITE)
            t3 = self.font_boss_sm.render(
                "Vá ao mapa e entre no portal do Monstro do Cálculo!",
                True, (200,220,200))

            s.blit(t1, (box.centerx - t1.get_width()//2, box.y + 8))
        ...
        for btn, lbl, c in [
            ...
            s.blit(t, (btn.centerx - t.get_width()//2,
